...
const char deployment[] = "Bg2g3g4g5 Ci3i4i5 Cd0e0f0 Da2a3 Dd7d8 Dc2c3 Sa5 Sb7 Sg8 Sc5 ";
...
int dd[4] = {0};
int testint = 0;
int testintarr[TESTSIZE][2] = { };//{0,5},{3,3},{1,1},{0,2},{2,5},{4,5},{6,3},{1,3} };//{{0,5},{2,5},{4,5},{6,3},{3,3},{0,3}}; //{6,8},{6,0},{8,6},{8,2} , {0,2},{0,6},{2,8},{2,0}}; //, {2,2},{6,2},{2,6},{6,6}, {4,4} };
...
void respond_with_shot(void)
{
  ...
  while (TRUE)
	...
	for (i=0;i<9;i++){
	  for (j=0;j<9;j++){
	    if ( (i-1<0 || (i-1>=0 && (enemy_board[i-1][j] == NOSHIP || enemy_board[i-1][j] == ROCK))) && (j+1>=9 || (j+1<9 && (enemy_board[i][j+1] == NOSHIP || enemy_board[i][j+1] == ROCK))) && (i+1>=9 || (i+1<9 && (enemy_board[i+1][j] == NOSHIP || enemy_board[i+1][j] == ROCK))) && (j-1<0 || (j-1>=0 && (enemy_board[i][j-1] == NOSHIP || enemy_board[i][j-1] == ROCK))) ){
	...
	else if (2*bb[6] == bb[4] + bb[2] && 2*bb[7] == bb[5] + bb[3]){
	  bb[0] = x3;
	  bb[1] = y3;
	  x = bb[0];
	  y = bb[1];
